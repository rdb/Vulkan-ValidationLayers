...
static bool PreCallValidateCreateQueryPool(layer_data *dev_data) {
    return log_msg(dev_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_QUERY_POOL_EXT, 0,
...
VKAPI_ATTR VkResult VKAPI_CALL CreateQueryPool(VkDevice device, const VkQueryPoolCreateInfo *pCreateInfo,
    ...
    if (pCreateInfo && pCreateInfo->queryType == VK_QUERY_TYPE_PIPELINE_STATISTICS) {
        if (!dev_data->enabled_features2.features.pipelineStatisticsQuery) {
            skip |= PreCallValidateCreateQueryPool(dev_data);
        }
    }
    lock.unlock();
...
VKAPI_ATTR VkResult VKAPI_CALL BeginCommandBuffer(VkCommandBuffer commandBuffer, const VkCommandBufferBeginInfo *pBeginInfo) {
    ...
    if (cb_node) {
        skip |= PreCallValidateBeginCommandBuffer(dev_data, cb_node, commandBuffer, pBeginInfo);
        PreCallRecordBeginCommandBuffer(dev_data, cb_node, commandBuffer, pBeginInfo);
    }
    lock.unlock();
    if (skip) {
        return VK_ERROR_VALIDATION_FAILED_EXT;
...
static bool PreCallValidateCmdBeginRenderPass(layer_data *dev_data, const RENDER_PASS_STATE *render_pass_state, GLOBAL_CB_NODE *cb_state,
...
static void PreCallRecordCmdBeginRenderPass(layer_data *dev_data, RENDER_PASS_STATE *render_pass_state,
                                            GLOBAL_CB_NODE *cb_state, FRAMEBUFFER_STATE *framebuffer,
...
VKAPI_ATTR void VKAPI_CALL CmdBeginRenderPass(VkCommandBuffer commandBuffer, const VkRenderPassBeginInfo *pRenderPassBegin,
    ...
    if (cb_node) {
        skip |= PreCallValidateCmdBeginRenderPass(dev_data, render_pass_state, cb_node, framebuffer, pRenderPassBegin);
        if (render_pass_state) {
            PreCallRecordCmdBeginRenderPass(dev_data, render_pass_state, cb_node, framebuffer, pRenderPassBegin, contents);
        }
    }
    lock.unlock();
    if (!skip) {
...
static bool PreCallValidateUnmapMemory(const layer_data *dev_data, const VkDeviceMemory &mem) {
...
VKAPI_ATTR void VKAPI_CALL UnmapMemory(VkDevice device, VkDeviceMemory mem) {
    ...
    if (mem_info) {
        if (!mem_info->mem_range.size) {
            ...
            skip |= PreCallValidateUnmapMemory(dev_data, mem);
        }
